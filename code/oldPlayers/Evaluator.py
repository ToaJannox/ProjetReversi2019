...
class Evaluator:
    ...
    def eval(board, player):
        ...
        if board.is_game_over():
            return 1000 * Evaluator._eval_disc_diff(board, player)

        game_phase = Evaluator._get_game_phase(board)

        if game_phase == GamePhase.EARLY_GAME:
            return 1000 * Evaluator._eval_corner(board, player) + 50 * Evaluator._eval_mobility(board, player)
        elif game_phase == GamePhase.MID_GAME:
            return 1000 * Evaluator._eval_corner(board, player) + 20 * Evaluator._eval_mobility(board, player) + 10\
                   * Evaluator._eval_disc_diff(board, player) + 100 * Evaluator._eval_parity(board)
        else:  # game_phase == GamePhase.LATE_GAME:
            return 1000 * Evaluator._eval_corner(board, player) + 100 * Evaluator._eval_mobility(board, player) + 500\
                   * Evaluator._eval_disc_diff(board, player) + 500 * Evaluator._eval_parity(board)
